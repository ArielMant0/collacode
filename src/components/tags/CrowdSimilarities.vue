...
<script setup>
    ...
    const props = defineProps({
        ...
        zIndex: { type: Number, default: 2999 }
    })

    const el = useTemplateRef("el")
    const elSize = useElementSize(el)
    const graphWidth = computed(() => Math.max(300, elSize.width.value * 0.95))
    const graphHeight = computed(() => Math.max(300, elSize.height.value * 0.925-350))
    ...
    async function read() {
        ...
        let data;
        // get similarity data
        if (!DM.hasData("similarity")) {
            data = await getSimilarities()
            DM.setData("similarity", data)
        } else {
            data = DM.getData("similarity", false)
        }
