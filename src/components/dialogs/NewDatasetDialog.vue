<template>
    <MiniDialog v-model="model" :title="title" min-width="900" close-icon>
        ...
        <template v-slot:actions>
            ...
            <v-btn class="ml-2" color="success" @click="submit">create</v-btn>
        </template>
    </MiniDialog>
</template>
        ...
        try {
            ...
            if (hasScheme.value && columns.value.length > 0) {
                ...
                for (let i = 0; i < columns.value.length; ++i) {
                    ...
                    if (c.name.length === 0) {
                        return toast.error("missing data field name")
                    }
                    if (columns.value.some((d, j) => i !== j && d.name === c.name)) {
                        return toast.error(`duplicate data field name "${c.name}"`)
                    }
            ...
            await addDataset(obj)
            toast.success("created project " + name.value)
            emit("submit", obj)
            model.value = false;
            times.needsReload("datasets")
            reset()
        } catch {
            toast.error("error creating project " + name.value)
        }
    }
