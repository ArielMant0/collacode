...
def get_meta_scheme(cur, dataset, path, backup_path):
    res = cur.execute(
        f"SELECT meta_scheme FROM {TBL_DATASETS} WHERE id = ?;", (dataset,)
    ).fetchone()
    if res is None:
        return None
...
def add_dataset(cur, obj, path, backup_path):
    ...
    if "scheme" in obj:
        ...
        stmt = (
            f"CREATE TABLE {obj['meta_table']} (id INTEGER PRIMARY KEY, item_id INTEGER NOT NULL, "
        )
        ...
        cur.execute(stmt)
        cur.execute(
            f"UPDATE {TBL_DATASETS} SET meta_table = ?, meta_scheme = ? WHERE id = ?;",
            (obj["meta_table"], obj["meta_scheme"], id),
        )
...
def get_items_by_dataset(cur, dataset, path, backup_path):
    ...
    if tbl_name is None:
        return cur.execute(
            f"SELECT * FROM {TBL_ITEMS} WHERE dataset_id = ?;", (dataset,)
        ).fetchall()
...
def add_users(cur, dataset, data):
    ...
    stmt = (
        f"INSERT INTO {TBL_USERS} (dataset_id, name, role, email) VALUES (?, ?, ?, ?);"
        if not with_id
        else f"INSERT INTO {TBL_USERS} (id, dataset_id, name, role, email) VALUES (?, ?, ?, ?, ?);"
    )
