...
def upgrade(connection):
    ...
    for e in data:
        ...
        for t in e["tags"]:
            ev = (
                ev
                + cur.execute(
                    "SELECT * FROM evidence WHERE game_id = ? AND tag_id = ? AND code_id = ?;",
                    (e["game_id"], t["tag_id"], e["code_id"]),
                ).fetchall()
            )
    ...
    for e in data:
        ...
        group = cur.execute(
            "INSERT INTO ext_groups (game_id, code_id, created, created_by) VALUES (:game_id, :code_id, :created, :created_by) RETURNING id;",
            e,
        ).fetchone()
        ...
        for d in e["evidence"]:
            cur.execute(
                "INSERT OR REPLACE INTO ext_ev_connections (ev_id, ext_id) VALUES (?, ?);",
                (d, e["id"]),
            )
...
def downgrade(connection):
    ...
    for e in data:
        ...
        for d in e["categories"]:
            cur.execute(
                "INSERT OR IGNORE INTO ext_cat_connections (id, cat_id, ext_id) VALUES (:id, :cat_id, :ext_id);",
                d,
            )
        ...
        for d in e["tags"]:
            cur.execute(
                "INSERT OR IGNORE INTO ext_tag_connections (id, tag_id, ext_id) VALUES (:id, :tag_id, :ext_id);",
                d,
            )
        ...
        for d in e["agreements"]:
            cur.execute(
                "INSERT OR IGNORE INTO ext_agreements (id, ext_id, created_by, value) VALUES (:id, :ext_id, :created_by, :value);",
                d,
            )
