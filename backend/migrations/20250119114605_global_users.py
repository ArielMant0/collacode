...
def upgrade(connection):
    ...
    cur.execute("""CREATE TABLE project_users (
        id	INTEGER PRIMARY KEY,
    ...
    cur.execute("""CREATE TABLE update_times (
        id	INTEGER PRIMARY KEY,
        name	TEXT NOT NULL,
        dataset_id	INTEGER NOT NULL,
...
def downgrade(connection):
    ...
    # get all users
    users = cur.execute("SELECT * FROM project_users du INNER JOIN users u ON du.user_id = u.id;").fetchall()
    updates = cur.execute("SELECT * FROM update_times;").fetchall()
